...
nth_prog->n_times_used=nth_prog->n_times_used+1;/*added to increment the popularity through list*/
}
else {
...
static void run_program(char *c) 
{
	...
	if( must_wait ) 
	{
		...
		ppid=wait(&status);//parent process in wait state for child process to complete.
		return ;
	}
	else
	{	
